...
Total Experience: ~3.5+ years (KPMG & GlobalLogic).

Core Stack & Expertise:
- Microsoft Power Platform: Power Apps, Power Automate, Power BI, SharePoint Online List Migrations.
- GenAI & Agentic Systems: Copilot Studio, Multi-modal Copilot Agents, Azure AI Fundamentals (AI-901), Oracle Agentic AI Certified.
- Operations Optimization: Lean Six Sigma Yellow Belt, VBA Macros, SQL, QA Management, Governance.
- Stakeholder Engagement: 360-degree stakeholder management across 13 sectors, 100+ RFP/RFI responses.

Key Accomplishments:
- KPMG: Built Power Platform solutions handling 20,000 interactions annually across 13 sectors, saving 1,200 hrs/yr.
- KPMG: Deployed multi-modal Copilot agent for automated metadata tagging, saving 325 hrs/yr.
- KPMG: Saved 2,000+ total hours annually using Power Platform & VBA macros. Won KUDOS & Gurus@Work.
- GlobalLogic: Designed QA frameworks for Google GenAI datasets for Android search, elevating quality from 74% to 95%.
"""
...
def init_tracker():
    ...
    if not os.path.exists(EXCEL_FILE):
        df = pd.DataFrame(columns=[
            ...
            "Status", "Tailored Resume Summary", "Tailored Bullets", "Cover Letter"
        ])
...
def log_job(url, title, company, location, ctc, score, summary, bullets, cover_letter):
    conn = sqlite3.connect("job_tracker.db")
    c = conn.cursor()
    ...
    try:
        ...
        new_row = {
            ...
            "Tailored Resume Summary": summary,
            "Tailored Bullets": "\n".join(bullets),
...
def generate_application_kit(title, company, description):
    prompt = f"""
    You are an executive career coach tailoring high-impact job materials for Kartik Bhatt.
    ...
    Generate a complete, targeted application package.
    Return ONLY a JSON object with this structure:
    {{
        "match_score": "e.g., 92%",
        "reason": "1-2 sentences on why Kartik's exact tech stack fits this role.",
        "skills_gap": "Any missing tool/skill or 'None'",
        "tailored_summary": "A 3-line tailored Professional Summary for the top of Kartik's resume specifically customized for this JD.",
        "tailored_experience_bullets": [
            "Tailored bullet 1 highlighting relevant impact (KPMG Power Platform / Copilot)",
            "Tailored bullet 2 highlighting relevant impact (GlobalLogic GenAI QA / Process Automation)",
            "Tailored bullet 3 highlighting stakeholder & operational delivery"
        ],
        "cover_letter": "A concise, 3-paragraph (under 180 words) tailored cover letter ready for copy-pasting."
    }}
    """
    try:
        ...
        return {
            ...
            "reason": "Strong alignment with Kartik's Power Platform and GenAI agent background.",
            "skills_gap": "None",
            ...
            "tailored_experience_bullets": [
                "Architected Power Platform and Copilot Studio solutions saving 2,000+ hours annually across 13 sectors at KPMG.",
                "Engineered robust QA processes for Google GenAI datasets at GlobalLogic, increasing quality from 74% to 95%."
            ],
...
def send_telegram_alert(title, company, location, url, ctc_label, kit):
    bullets_text = "\n".join([f"• {b}" for b in kit.get("tailored_experience_bullets", [])])
    
    # Message 1: Job Match Overview & Tailored Resume
    msg_1 = (
        ...
        f"⚡ *Tailored Experience Bullets:*\n{bullets_text}\n\n"
        f"🔗 [Apply Directly Here]({url})"
    )

    # Message 2: Ready-to-copy Cover Letter
    msg_2 = (
        f"📝 *Tailored Cover Letter for {company}:*\n```\n{kit.get('cover_letter')}\n```"
    )
...
def run():
    ...
    for job in all_jobs:
        ...
        send_telegram_alert(title, company, location, url, ctc_label, kit)
        ...
        log_job(
            ...
            summary=kit.get("tailored_summary"),
            bullets=kit.get("tailored_experience_bullets", []),
            cover_letter=kit.get("cover_letter")
        )
        print(f"Dispatched kit for {title} at {company}")
